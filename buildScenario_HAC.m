...
clear
close all
% Programa para extrair cenários com o hierarchical agglomerative clustering (HAC)
%
%% Leitura das séries
...
%
%% Gerar cenários
ncen = 10;  % Número de cenários (Parâmetro do HAC)
[nobs, nser] = size(data);  % Número de observações e séries em data
%
% HAC
Z = linkage(data,'ward','euclidean');
idx = cluster(Z,'maxclust',ncen);  % Lista de índices de cenários para cada observação
%
...
disp(grouped_data)
%
csvwrite('results/grouped_data_HAC.csv',grouped_data)
disp('The results were saved in results/grouped_data_HAC.csv')
